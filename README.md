...
- Minimum allocation of 4GB for the "Enclave Page Cache Size"
...
in [Intel's how-to guide][intel-dcap-install-howto]. After subscribing to the service, you will get two keys: a primary
API key and a secondary API key.
...
- `./manage configure` - generate a configuration files (.env, certificates, etc.)
- `./manage register`  - register a node onchain (can be used for re-registration)
- `./manage verify`    - verify that a raiko instance is running properly
